...
final class CodingTests: XCTestCase {
  ...
  lazy var memberThree = SWIM.Member(
    peer: nioPeer, status: .alive(incarnation: 2), protocolPeriod: 0)
  ...
  func test_serializationOf_node() throws {
    ...
    try self.shared_serializationRoundtrip(
      ContainsNode(
        node: Node(protocol: "udp", host: "127.0.0.1", port: 1111, uid: .random(in: 0...UInt64.max))
      )
    )
    try self.shared_serializationRoundtrip(
    ...
    try self.shared_serializationRoundtrip(
      Node(
        protocol: "udp", name: "kappa", host: "127.0.0.1", port: 2222,
        uid: .random(in: 0...UInt64.max))
    )
  }
  ...
  func test_serializationOf_pingReq() throws {
    ...
    try self.shared_serializationRoundtrip(
      SWIM.Message.pingRequest(
        target: self.nioPeer, replyTo: self.nioPeerOther, payload: payloadNone, sequenceNumber: 111)
    )
    ...
    try self.shared_serializationRoundtrip(
      SWIM.Message.pingRequest(
        target: self.nioPeer, replyTo: self.nioPeerOther, payload: payloadSome, sequenceNumber: 1212
      ))
  }
