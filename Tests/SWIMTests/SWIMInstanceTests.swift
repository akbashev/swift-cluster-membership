...
final class SWIMInstanceTests: XCTestCase {
  ...
  func test_notMyself_shouldDetectRemoteVersionOfSelf() {
    let swim = SWIM.Instance<TestPeer, TestPeer, TestPeer>(
      settings: SWIM.Settings(), myself: self.myself)
  ...
  func test_mark_shouldNotApplyEqualStatus() throws {
    ...
    var swim = SWIM.Instance<TestPeer, TestPeer, TestPeer>(
      settings: SWIM.Settings(), myself: self.myself)
    ...
    try self.validateMark(
      swim: &swim, peer: otherPeer, status: .suspect(incarnation: 1, suspectedBy: [self.thirdNode]),
      shouldSucceed: false)
  ...
  func test_mark_shouldApplyNewerStatus() throws {
    ...
    for i: SWIM.Incarnation in 0...5 {
      swim.incrementProtocolPeriod()
      try self.validateMark(
        swim: &swim, peer: otherPeer,
        status: .suspect(incarnation: SWIM.Incarnation(i), suspectedBy: [self.thirdNode]),
        shouldSucceed: true)
      try self.validateMark(
        swim: &swim, peer: otherPeer, status: .alive(incarnation: SWIM.Incarnation(i + 1)),
        shouldSucceed: true)
    }
  ...
  func test_mark_shouldNotApplyOlderStatus_suspect() throws {
    ...
    _ = swim.addMember(
      suspectMember, status: .suspect(incarnation: 1, suspectedBy: [self.thirdNode]))
    swim.incrementProtocolPeriod()

    try self.validateMark(
      swim: &swim, peer: suspectMember,
      status: .suspect(incarnation: 0, suspectedBy: [self.thirdNode]), shouldSucceed: false)
    try self.validateMark(
      swim: &swim, peer: suspectMember, status: .alive(incarnation: 1), shouldSucceed: false)
  ...
  func test_mark_shouldNotApplyOlderStatus_unreachable() throws {
    ...
    try self.validateMark(
      swim: &swim, peer: unreachableMember,
      status: .suspect(incarnation: 0, suspectedBy: [self.thirdNode]), shouldSucceed: false)
    try self.validateMark(
      swim: &swim, peer: unreachableMember, status: .alive(incarnation: 1), shouldSucceed: false)
  ...
  func test_mark_shouldNotApplyAnyStatusIfAlreadyDead() throws {
    ...
    try self.validateMark(
      swim: &swim, peer: otherPeer, status: .alive(incarnation: 99), shouldSucceed: false)
    try self.validateMark(
      swim: &swim, peer: otherPeer,
      status: .suspect(incarnation: 99, suspectedBy: [self.thirdNode]), shouldSucceed: false)
  ...
  func test_onPingRequestResponse_allowsSuspectNodeToRefuteSuspicion() {
    ...
    _ = swim.onPingRequestResponse(
      .ack(target: thirdPeer, incarnation: 2, payload: .none, sequenceNumber: 1), pinged: thirdPeer)
  ...
  func test_onPingRequestResponse_ignoresTooOldRefutations() {
    ...
    // and now we get an `ack` back, secondPeer claims that thirdPeer is indeed alive!
    _ = swim.onPingRequestResponse(
      .ack(target: thirdPeer, incarnation: 1, payload: .none, sequenceNumber: 1), pinged: thirdPeer)
    // may print the result for debugging purposes if one wanted to
  ...
  func test_onPingRequestResponse_storeIndividualSuspicions() throws {
    ...
    _ = swim.addMember(
      self.second, status: .suspect(incarnation: 1, suspectedBy: [self.secondNode]))

    _ = swim.onPingRequestResponse(
      .timeout(
        target: self.second, pingRequestOrigin: nil, timeout: .milliseconds(800), sequenceNumber: 1),
      pinged: self.second)
  ...
  func test_MarkedDirective_isEffectiveChange() {
    ...
    XCTAssertTrue(
      SWIM.MemberStatusChangedEvent(
        previousStatus: nil,
        member: SWIM.Member(
          peer: p, status: .suspect(incarnation: 1, suspectedBy: [self.thirdNode]),
          protocolPeriod: 1)
      )
      .isReachabilityChange)
    XCTAssertTrue(
    ...
    XCTAssertTrue(
      SWIM.MemberStatusChangedEvent(
        previousStatus: nil, member: SWIM.Member(peer: p, status: .dead, protocolPeriod: 1)
      )
      .isReachabilityChange)
    ...
    XCTAssertFalse(
      SWIM.MemberStatusChangedEvent(
        previousStatus: .suspect(incarnation: 1, suspectedBy: [self.thirdNode]),
        member: SWIM.Member(
          peer: p, status: .suspect(incarnation: 2, suspectedBy: [self.thirdNode]),
          protocolPeriod: 1)
      )
      .isReachabilityChange)
    XCTAssertTrue(
    ...
    XCTAssertFalse(
      SWIM.MemberStatusChangedEvent(
        previousStatus: .dead, member: SWIM.Member(peer: p, status: .dead, protocolPeriod: 1)
      )
      .isReachabilityChange)
  }
  ...
  func test_onGossipPayload_myself_withSuspectAndLowerIncarnation_shouldNotIncrementIncarnation()
    throws
  {
    var swim = SWIM.Instance<TestPeer, TestPeer, TestPeer>(
      settings: SWIM.Settings(), myself: self.myself)
    ...
    myselfMember.status = .suspect(
      incarnation: currentIncarnation - 1, suspectedBy: [self.thirdNode])  // purposefully "previous"
    let directives = swim.onGossipPayload(about: myselfMember)
  ...
  func test_onGossipPayload_myself_withSuspectAndHigherIncarnation_shouldNotIncrementIncarnation()
    throws
  {
    var swim = SWIM.Instance<TestPeer, TestPeer, TestPeer>(
      settings: SWIM.Settings(), myself: self.myself)
    ...
    myselfMember.status = .suspect(
      incarnation: currentIncarnation + 6, suspectedBy: [self.thirdNode])
    let directives = swim.onGossipPayload(about: myselfMember)
  ...
  func test_onGossipPayload_other_withUnreachable_unreachabilityEnabled() throws {
    ...
    default:
      XCTFail(
        "Expected `.applied(.some(change to unreachable))`, got: \(String(reflecting: directives))")
    }
  }
  ...
  func test_onGossipPayload_other_withNewSuspicion_shouldStoreIndividualSuspicions() throws {
      ...
      case .suspect(_, let confirmations) = change.status
    {
      ...
      XCTAssertTrue(
        confirmations.contains(secondNode), "expected \(confirmations) to contain \(secondNode)")
      XCTAssertTrue(
        confirmations.contains(thirdNode), "expected \(confirmations) to contain \(thirdNode)")
    } else {
  ...
  func test_onGossipPayload_other_shouldNotApplyGossip_whenHaveEnoughSuspectedBy() throws {
    ...
    _ = swim.addMember(
      other, status: .suspect(incarnation: 0, suspectedBy: Set(saturatedSuspectedByList)))
  ...
  func test_onGossipPayload_other_shouldNotExceedMaximumSuspectedBy() throws {
    ...
    _ = swim.addMember(
      other, status: .suspect(incarnation: 0, suspectedBy: [self.thirdNode, self.secondNode]))
  ...
  func test_onPingRequestResponse_incrementLHAMultiplier_whenMissedNack() {
    ...
    _ = swim.onEveryPingRequestResponse(
      .timeout(
        target: secondPeer, pingRequestOrigin: nil, timeout: .milliseconds(300), sequenceNumber: 1),
      pinged: secondPeer)
  ...
  func test_onPingRequestResponse_handlesNacksCorrectly() {
    ...
    _ = swim.addMember(
      self.fourth, status: .suspect(incarnation: 0, suspectedBy: [self.third.node]))
    ...
    _ = swim.onPingRequestResponse(
      .timeout(
        target: self.fourth, pingRequestOrigin: nil, timeout: .nanoseconds(1), sequenceNumber: 2),
      pinged: self.fourth
    )
    XCTAssertEqual(swim.localHealthMultiplier, 0)
    ...
    _ = swim.onPingRequestResponse(
      .timeout(
        target: self.fourth, pingRequestOrigin: nil, timeout: .nanoseconds(1), sequenceNumber: 3),
      pinged: self.fourth
    )
    XCTAssertEqual(swim.localHealthMultiplier, 0)
  }
  ...
  func test_onPingRequestResponse_notIncrementLHAMultiplier_whenSeeOldSuspicion_onGossip() {
    ...
    _ = swim.onGossipPayload(
      about: SWIM.Member(
        peer: p1, status: .suspect(incarnation: 0, suspectedBy: [self.thirdNode]), protocolPeriod: 0
      ))
    XCTAssertEqual(swim.localHealthMultiplier, 1)
  ...
  func test_onPingRequestResponse_dontChangeLHAMultiplier_whenGotNack() {
    ...
    _ = swim.onEveryPingRequestResponse(
      .nack(target: secondPeer, sequenceNumber: 1), pinged: secondPeer)
    XCTAssertEqual(swim.localHealthMultiplier, 1)
  }
  ...
  func test_addMember_shouldReplaceMemberIfDifferentUID() {
    ...
    default:
      XCTFail(
        "Expected replacement directive, was: \(optional: directives.first), in: \(directives)")
    }
    ...
    default:
      XCTFail(
        "Expected .added as directive, was: \(optional: directives.dropFirst().first), in: \(directives)"
      )
    }
  ...
  func test_member_shouldReturnTheLastAssignedStatus() {
    ...
    XCTAssertEqual(
      swim.member(for: otherPeer)!.status, .suspect(incarnation: 99, suspectedBy: [self.thirdNode]))
  }
  ...
  func test_suspects_shouldContainOnlySuspectedNodes() {
    ...
    let directive: SWIM.Instance.MarkedDirective = swim.mark(
      self.second, as: .suspect(incarnation: 0, suspectedBy: [self.third.node]))
    switch directive {
  ...
  func test_suspects_shouldMark_whenBiggerSuspicionList() {
    ...
    self.validateSuspects(swim, expected: [self.second.node])
    let newStatus: SWIM.Status = .suspect(
      incarnation: 0, suspectedBy: [self.thirdNode, self.secondNode])
  ...
  func test_suspects_shouldNotMark_whenSmallerSuspicionList() {
    ...
    self.validateSuspects(swim, expected: [])
    let oldStatus: SWIM.Status = .suspect(
      incarnation: 0, suspectedBy: [self.thirdNode, self.secondNode])
    ...
    XCTAssertEqual(
      swim.mark(self.second, as: newStatus), .ignoredDueToOlderStatus(currentStatus: oldStatus))
    let d2 = swim.mark(self.second, as: newStatus)
    switch d2 {
  ...
  func test_makeGossipPayload_shouldReset_whenNewMemberChangedStatus() throws {
    ...
    _ = swim.addMember(self.third, status: .alive(incarnation: 0))
    let myselfMember = SWIM.Member(
      peer: self.myself, status: .alive(incarnation: 0), protocolPeriod: 0)
    let thirdMember = SWIM.Member(
      peer: self.third, status: .alive(incarnation: 0), protocolPeriod: 0)

    try self.validateGossip(
      swim: &swim,
      expected: [
        .init(peer: self.second, status: .alive(incarnation: 0), protocolPeriod: 0), myselfMember,
        thirdMember,
      ])
    ...
    try self.validateGossip(
      ...
      expected: [
        .init(
          peer: self.second, status: .suspect(incarnation: 0, suspectedBy: [self.thirdNode]),
          protocolPeriod: 0),
        myselfMember,
        thirdMember,
      ])
  ...
  func test_confirmDead_tombstone_shouldExpireAfterConfiguredAmountOfTicks() throws {
    ...
    XCTAssertTrue(
      swim.removedDeadMemberTombstones
        .contains(
          .init(uid: self.secondNode.uid!, deadlineProtocolPeriod: 0 /* not part of equality*/))
    )
  ...
  func validateMark(
    swim: inout SWIM.Instance<TestPeer, TestPeer, TestPeer>, member: SWIM.Member<TestPeer>,
    status: SWIM.Status, shouldSucceed: Bool,
    file: StaticString = (#file), line: UInt = #line
  ) throws {
    try self.validateMark(
      swim: &swim, peer: member.peer, status: status, shouldSucceed: shouldSucceed, file: file,
      line: line)
  }

  func validateMark(
    swim: inout SWIM.Instance<TestPeer, TestPeer, TestPeer>, peer: TestPeer, status: SWIM.Status,
    shouldSucceed: Bool,
    file: StaticString = (#file), line: UInt = #line
  ) throws {
  ...
  func validateGossip(
    swim: inout SWIM.Instance<TestPeer, TestPeer, TestPeer>, expected: Set<SWIM.Member<TestPeer>>,
    file: StaticString = (#file), line: UInt = #line
  ) throws {
