...
final class SWIMInstanceTests: XCTestCase {
  ...
  func test_onPingRequestResponse_storeIndividualSuspicions() throws {
    ...
    _ = swim.addMember(
      self.second,
      status: .suspect(incarnation: 1, suspectedBy: [self.secondNode])
    )
    ...
    throws
  {
    ...
    myselfMember.status = .suspect(
      incarnation: currentIncarnation - 1,
      suspectedBy: [self.thirdNode]
    )  // purposefully "previous"
    let directives = swim.onGossipPayload(about: myselfMember)
    ...
    throws
  {
    ...
    myselfMember.status = .suspect(
      incarnation: currentIncarnation + 6,
      suspectedBy: [self.thirdNode]
    )
    let directives = swim.onGossipPayload(about: myselfMember)
  ...
  func test_onGossipPayload_other_withNewSuspicion_shouldStoreIndividualSuspicions() throws {
      ...
      case .suspect(_, let confirmations) = change.status
    {
      ...
      XCTAssertTrue(
        confirmations.contains(secondNode),
        "expected \(confirmations) to contain \(secondNode)"
      )
      XCTAssertTrue(
        confirmations.contains(thirdNode),
        "expected \(confirmations) to contain \(thirdNode)"
      )
    } else {
  ...
  func test_onGossipPayload_other_shouldNotApplyGossip_whenHaveEnoughSuspectedBy() throws {
    ...
    _ = swim.addMember(
      other,
      status: .suspect(incarnation: 0, suspectedBy: Set(saturatedSuspectedByList))
    )
  ...
  func test_onGossipPayload_other_shouldNotExceedMaximumSuspectedBy() throws {
    ...
    _ = swim.addMember(
      other,
      status: .suspect(incarnation: 0, suspectedBy: [self.thirdNode, self.secondNode])
    )
  ...
  func test_onPingRequestResponse_handlesNacksCorrectly() {
    ...
    _ = swim.addMember(
      self.fourth,
      status: .suspect(incarnation: 0, suspectedBy: [self.third.node])
    )
  ...
  func test_onPingRequestResponse_dontChangeLHAMultiplier_whenGotNack() {
    ...
    _ = swim.onEveryPingRequestResponse(
      .nack(target: secondPeer, sequenceNumber: 1),
      pinged: secondPeer
    )
    XCTAssertEqual(swim.localHealthMultiplier, 1)
  }
  ...
  func test_member_shouldReturnTheLastAssignedStatus() {
    ...
    XCTAssertEqual(
      swim.member(for: otherPeer)!.status,
      .suspect(incarnation: 99, suspectedBy: [self.thirdNode])
    )
  }
  ...
  func test_suspects_shouldMark_whenBiggerSuspicionList() {
    ...
    let newStatus: SWIM.Status = .suspect(
      incarnation: 0,
      suspectedBy: [self.thirdNode, self.secondNode]
    )
  ...
  func test_suspects_shouldNotMark_whenSmallerSuspicionList() {
    ...
    XCTAssertEqual(
      swim.mark(self.second, as: newStatus),
      .ignoredDueToOlderStatus(currentStatus: oldStatus)
    )
    let d2 = swim.mark(self.second, as: newStatus)
    switch d2 {
